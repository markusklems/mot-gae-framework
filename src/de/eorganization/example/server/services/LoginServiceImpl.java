package de.eorganization.example.server.services;
...
import de.eorganization.example.server.OAuth2Provider;
import de.eorganization.example.server.servlets.util.CookiesUtil;
...
public abstract class LoginServiceImpl extends RemoteServiceServlet implements
		LoginService {
		...
		if (oauthService != null) {
			try {
				...
				if (OAuth2Provider.GOOGLE.equals(provider)) {
					...
					Member tempMember = findMemberBySocialId(new Integer(twitterUserInfo
									.getInt("id")).toString());
					if (tempMember == null) {
						tempMember = new Member();
			...
			if (userService.isUserLoggedIn() && user != null) {
				loginInfo.setLoggedIn(true);
				loginInfo.setMember(updateMember(user));
	...
	@Override
	public abstract Member updateMember(User user);

	public abstract Member findMemberByFilter(Map<String, Object> filter);

	public abstract Member findMemberBySocialId(String socialId);
